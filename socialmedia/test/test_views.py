...
@pytest.mark.django_db()
def test_delete_social_media_not_exists(common_user_token):
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION="Token " + common_user_token)
    ...
    url = reverse("delete-social-media",kwargs={"social_media_id": 999})
    response = client.delete(url)
    assert response.status_code == 404
    
@pytest.mark.django_db()
def test_delete_social_media_anonnymus_user():
    client = APIClient()
